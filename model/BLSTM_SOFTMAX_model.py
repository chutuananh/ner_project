...
embed_char = TimeDistributed(Embedding(len(char_to_int), 30, embeddings_initializer = RandomUniform(minval = -0.5, maxval = 0.5)), name = 'char_embedding')(char_input)
...
output = TimeDistributed(Dense(len(label_to_int), activation = 'softmax'))(output)
...
with open("BLSTM_SOFTMAX_model_v0.json", "w") as json_file:
    json_file.write(model_json)

model.save_weights("BLSTM_SOFTMAX_weights_v0.h5")
...
with open('BLSTM_SOFTMAX_loss_v0.txt', 'w') as f:
	f.writelines("%f\n" % i for i in train_loss_list)
	f.writelines("\n")
	f.writelines("%f\n" % i for i in dev_loss_list)

with open('BLSTM_SOFTMAX_f1score_v0.txt', 'w') as f:
